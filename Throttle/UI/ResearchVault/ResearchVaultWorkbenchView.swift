...
extension ResearchVaultWorkbenchModel {
    ...
    private static func projectSegment(under root: URL) -> Int {
    ...
    private static func projectNames(under root: URL, segment: Int) -> Set<String> {
    ...
    private func syncFolderSource(
        ...
        for (projectKey, files) in byProject.sorted(by: { $0.key < $1.key }) {
            for chunk in files.chunked(into: ManualResearchFileImporter.maximumDocuments) {
