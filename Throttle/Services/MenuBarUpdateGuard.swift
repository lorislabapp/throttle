...
enum MenuBarUpdateGuard {
    ...
    /// True once a runaway has been detected. `MenuBarLabel` renders a static
    /// icon in that case: no `Label`, no countdown, no width changes — nothing
    /// that can drive another `NSStatusItem._adjustLength` pass.
    ...
    private static func sample() {
        ...
        guard !isDegraded,
              ...
              grew >= softGrowthBytes else { return }

        lock.withLock { $0 = true }
        log.error("""
            menu-bar update runaway: footprint \(footprint / 1_048_576, privacy: .public) MB, \
            +\(grew / 1_048_576, privacy: .public) MB in one interval — freezing the menu-bar label
            """)
    }
