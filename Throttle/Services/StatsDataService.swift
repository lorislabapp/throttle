...
enum StatsDataService {
    ...
    /// The four raw token columns over a range, **grouped by family**.
    ///
    /// Grouped because whether the "API equivalent" figure is an upper bound is
    /// a rate-weighted comparison: output is under-charged while input and cache
    /// are over-charged, and summing the columns across families first would
    /// throw away the rates that decide it. See `PlanAdvisor.isUpperBound(_:)`.
    static func composition(
        in database: Database, range: Range, now: Date = Date()
    ) throws -> [ModelTier: PlanAdvisor.TokenComposition] {
        let cutoff = range.cutoff(now: now)
        let sql = """
            ...
            """
        let rows = cutoff > 0
            ? try Row.fetchAll(database, sql: sql, arguments: [cutoff])
            : try Row.fetchAll(database, sql: sql)
        var out: [ModelTier: PlanAdvisor.TokenComposition] = [:]
        for row in rows {
            guard let bucket: String = row["bucket"] else { continue }
            out[ModelTier(rawValue: bucket) ?? .other] = PlanAdvisor.TokenComposition(
                input: row["inp"] ?? 0,
                output: row["outp"] ?? 0,
