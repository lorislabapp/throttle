...
final class EdgeDeployServiceTests: XCTestCase {
    ...
    func testEdgeMCPDefinitionRoutesToBearerGatedStreamableEndpoint() throws {
        let data = try EdgeDeployService.edgeMCPDefinition(
            baseURL: "https://edge.example.ts.net:8787/", token: "secret-token")
        let object = try XCTUnwrap(
            JSONSerialization.jsonObject(with: data) as? [String: Any])

        XCTAssertEqual(object["type"] as? String, "http")
        XCTAssertEqual(object["url"] as? String, "https://edge.example.ts.net:8787/mcp")
    ...
    func testEdgeMCPDefinitionRejectsCleartextRemoteURL() {
        XCTAssertThrowsError(
            try EdgeDeployService.edgeMCPDefinition(
                baseURL: "http://100.64.0.12:8787/", token: "x"))
    }
}
