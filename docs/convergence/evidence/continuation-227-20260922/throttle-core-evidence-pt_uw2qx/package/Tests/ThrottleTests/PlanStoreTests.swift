...
// MARK: - Legacy wire format upgrade (not qualification of a historical binary)

extension PlanStoreTests {
    func testLegacyJournalRetainsBytesWhenNewVerificationReopensAsUnknown() throws {
        try writePlan(twoLeafPlan)
        let log = root.appendingPathComponent(".throttle/log/T1.1.ndjson")
        try FileManager.default.createDirectory(at: log.deletingLastPathComponent(),
                                                withIntermediateDirectories: true)
        // Synthetic pre-lease wire format: no eventID, mission, grant or receipt fields.
        let claim = #"{"at":"2026-09-18T10:00:00Z","by":"codex:legacy","seq":1,"type":"claimed"}"#
        let previous = SHA256.hash(data: Data(claim.utf8)).map { String(format: "%02x", $0) }.joined()
        let candidate = """
        {"at":"2026-09-18T10:01:00Z","by":"codex:legacy","prev":"\(previous)",\
        "seq":2,"type":"candidate_complete"}
        """
        let legacyBytes = Data((claim + "\n" + candidate + "\n").utf8)
        try legacyBytes.write(to: log)
        let store = PlanStore(projectRoot: root)
        XCTAssertTrue(try store.events(for: "T1.1").chainValid)
        XCTAssertEqual(try store.state(for: "T1.1").status, .candidate)
        XCTAssertNil(try store.state(for: "T1.1").pendingVerification)
        XCTAssertEqual(try Data(contentsOf: log), legacyBytes)

        let lease = try TaskVerificationLifecycle.begin(taskID: "T1.1", store: store,
            request: .init(command: "true", stamp: "task+base", author: "throttle:fixture", timeout: 1))
        let upgradedBytes = try Data(contentsOf: log)
        XCTAssertEqual(Data(upgradedBytes.prefix(legacyBytes.count)), legacyBytes)
        let reopened = PlanStore(projectRoot: root)
        XCTAssertTrue(try reopened.events(for: "T1.1").chainValid)
        XCTAssertEqual(try reopened.state(for: "T1.1").pendingVerification, lease)
        XCTAssertNil(try reopened.state(for: "T1.1").lastCheck)
        XCTAssertThrowsError(try TaskVerificationLifecycle.begin(taskID: "T1.1", store: reopened,
            request: .init(command: "true", stamp: "task+base", author: "throttle:fixture", timeout: 1),
            now: lease.expiresAt.addingTimeInterval(1))) {
            XCTAssertEqual($0 as? TaskVerificationError, .unresolvedExecution(lease.id))
        }
        XCTAssertEqual(try Data(contentsOf: log), upgradedBytes)
    }
}
