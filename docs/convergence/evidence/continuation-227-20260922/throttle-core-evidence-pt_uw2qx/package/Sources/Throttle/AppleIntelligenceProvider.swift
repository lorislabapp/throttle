...
struct AppleIntelligenceProvider: AIProvider {
    ...
    private func streamViaFoundationModels(
        ...
        let scope = AssistantProjectToolScope.current ?? AssistantProjectTools(projectPath: context.projectPath)
        let session = LanguageModelSession(
            tools: [
                ReadFileTool(scope: scope),
                ListFilesTool(scope: scope),
                SearchFilesTool(scope: scope)
            ],
