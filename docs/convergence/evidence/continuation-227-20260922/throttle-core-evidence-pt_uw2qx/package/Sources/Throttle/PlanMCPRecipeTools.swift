...
extension PlanMCPTools {
    ...
    static func claimText(
        ...
        authorizeMutation: @escaping () -> String? = { nil },
        runtimeAuthority: PlanMCPAuthority? = nil
    ) -> String {
        let request = ClaimRequest(
            ...
            author: author,
            missionID: missionID,
            retry: retry, authorizeMutation: authorizeMutation
        )
        do {
            return try store(project).mutate {
                if let refusal = authorizeMutation() { return refusal }
                return prepareClaim(request, store: $0)
            }
        } catch {
    ...
    private static func prepareClaim(_ request: ClaimRequest, store: PlanStore) -> String {
        ...
        var context = ClaimContext(
            ...
            plan: plan,
            retry: request.retry, authorizeMutation: request.authorizeMutation
        )
    ...
    private static func completeClaim(
        ...
        if let refusal = context.authorizeMutation() { return refusal }
