...
extension TaskIntegrationService {
    ...
    static func verify(
        ...
        guard let store, store.projectRoot == repo.standardizedFileURL.resolvingSymlinksInPath() else {
            throw TaskVerificationError.missingStore
        }
        ...
        let lease = try TaskVerificationLifecycle.begin(taskID: taskID, store: store,
            request: .init(command: command, stamp: stamp, author: author, timeout: timeout))
        let startedAt = Date()
        let result = verificationShell(command, in: worktree, timeout: timeout) { process in
            try TaskVerificationLifecycle.attachProcess(taskID: taskID, store: store, lease: lease, process: process)
        }
        guard result.exitObserved else { throw TaskVerificationError.processOutcomeUnknown }
        ...
        var receipt = WorkflowEvidenceReceipt.command(command, stamp: stamp,
            startedAt: startedAt, finishedAt: Date(), result: (result.succeeded, unchanged))
        ...
        receipt.untrackedDigest = untracked.ok
            ...
            : nil
        // Version inspection must not launch another xcodebuild alongside the
        // project's verification command or another session's compilation.
        let toolchain = shell("xcode-select -p", in: worktree, timeout: 30)
        receipt.environmentDigest = toolchain.succeeded
            ? verificationEnvironmentDigest(selectedDeveloperPath: toolchain.output)
            : nil
        ...
        let verdict = Verdict(passed: result.succeeded && unchanged && contractSatisfied,
                              output: boundedVerificationOutput(output),
                              stamp: stamp)

        let checked = TaskEvent(seq: 0, timestamp: Date(), author: author, type: .checked,
                                ref: stamp, reason: verdict.passed ? nil : verdict.output,
                                summary: command, passed: verdict.passed, receipt: receipt)
        let recorded = try TaskVerificationLifecycle.finish(taskID: taskID, store: store,
                                                            lease: lease, checked: checked)
        return recorded.passed == verdict.passed ? verdict
            : Verdict(passed: false, output: recorded.reason ?? verdict.output, stamp: stamp)
    }

    /// Read installed Xcode metadata without starting xcodebuild. Missing
    /// identity stays unknown; the selected path alone is not a version.
    private static func verificationEnvironmentDigest(selectedDeveloperPath: String) -> String? {
        let environment = ProcessInfo.processInfo.environment
        let path = environment["DEVELOPER_DIR"]
            ?? selectedDeveloperPath.trimmingCharacters(in: .whitespacesAndNewlines)
        let contents = URL(fileURLWithPath: path).deletingLastPathComponent()
        guard let data = try? Data(contentsOf: contents.appendingPathComponent("version.plist")),
              let version = try? PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any],
              let build = version["ProductBuildVersion"] as? String,
              let number = version["CFBundleShortVersionString"] as? String else { return nil }
        return WorkflowEvidenceReceipt.digest(environment: environment,
            toolchain: "\(path)\nXcode \(number)\nBuild version \(build)")
    }
    ...
    private static func verificationShell(
        _ command: String, in directory: URL, timeout: TimeInterval,
        attach: (TaskVerificationProcess) throws -> Void
    ) -> ShellResult {
        shell(command, in: directory, timeout: timeout) { child in
            guard let process = TaskVerificationProcess.captureAdmittedChild(child.pid) else {
                throw TaskVerificationError.processOutcomeUnknown
            }
            try attach(process)
        }
    }
    ...
    struct ShellResult {
        let succeeded: Bool
        let output: String
        var exitObserved = true
    }

    static func shell(
        _ command: String, in directory: URL,
        timeout: TimeInterval, beforeResume: (ChildControl) throws -> Void = { _ in }
    ) -> ShellResult {
        let child: ChildControl
        do {
            child = try spawn(command, in: directory, beforeResume: beforeResume)
        } catch {
            return ShellResult(succeeded: false, output: String(describing: error))
        }
        ...
        let sendTerm = DispatchWorkItem {
            child.signal(SIGTERM) { collector.markTimedOut() }
        }
    ...
    private static func verdictText(
        ...
        if collector.timedOut {
            ...
            case (true, true):
                output += " — its process group has no observed running members; detached descendants remain unknown."
            case (true, false):
        ...
        guard let status else {
            ...
            if let detail = child.waitFailure { output += "\n[throttle] \(detail)" }
            return ShellResult(succeeded: false, output: output, exitObserved: false)
        }
        return ShellResult(succeeded: status == 0 && !collector.timedOut, output: output)
    }
    ...
    private static let chdirWrapper = """
        IFS= read -r throttle_gate <&3 || exit 125
        [ "$throttle_gate" = "run" ] || exit 125
        exec 3<&-
    ...
    static func spawn(
        _ command: String, in directory: URL, beforeResume: (ChildControl) throws -> Void
    ) throws -> ChildControl {
        let gate = try TaskVerificationLaunchGate()
        var fds: [Int32] = [-1, -1]
        guard pipe(&fds) == 0 else { throw SpawnFailure(what: "pipe", code: errno) }
        let readFD = fds[0]
        let writeFD = fds[1]
        let pid: pid_t
        do { pid = try launch(command, in: directory, writeFD: writeFD, gateFD: gate.readDescriptor) } catch {
            close(writeFD)
            close(readFD)
            throw error
        }
        close(writeFD)
        let child = ChildControl(pid: pid, readFD: readFD)
        do {
            try beforeResume(child)
            try gate.release()
            return child
        } catch {
    ...
    private static func launch(
        ...
        try check("adddup2 admission", posix_spawn_file_actions_adddup2(&actions, gateFD, 3))
