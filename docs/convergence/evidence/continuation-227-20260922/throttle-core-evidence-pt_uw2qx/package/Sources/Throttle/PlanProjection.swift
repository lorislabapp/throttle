...
enum PlanProjection {
    ...
    private static func rejection(for event: TaskEvent, given state: TaskState) -> RejectedEvent.Reason? {
        ...
        if isVerificationEvent(event.type) {
            return verificationRejection(for: event, given: state)
        }
    ...
    private static func isVerificationEvent(_ type: TaskEventType) -> Bool {
        switch type {
        case .verificationStarted, .verificationProcessAttached, .verificationAbandoned, .checked:
            return true
        default:
            return false
        }
    ...
    private static func verificationRejection(
        for event: TaskEvent, given state: TaskState
    ) -> RejectedEvent.Reason? {
        if event.type == .verificationStarted {
            guard state.pendingVerification == nil, event.verificationLease?.isValid == true,
                  event.verificationLease?.fence == event.seq,
                  event.verificationLease?.owner == event.author else { return .staleVerification }
            return state.status == .candidate || state.status == .done ? nil : .terminal
        }
        if event.type == .verificationProcessAttached {
            return state.pendingVerification != nil && state.pendingVerification == event.verificationLease
                && state.verificationProcess == nil && event.verificationProcess?.isValid == true
                && event.author == state.pendingVerification?.owner ? nil : .staleVerification
        }
        if event.type == .verificationAbandoned {
            return state.pendingVerification != nil && state.pendingVerification == event.verificationLease
                ? nil : .staleVerification
        }
        if event.type == .checked {
            if let pending = state.pendingVerification, event.verificationLease != pending {
                return .staleVerification
            }
            if state.pendingVerification == nil, event.verificationLease != nil { return .staleVerification }
            return state.status == .candidate || state.status == .done ? nil : .terminal
        }
        return nil
    }
    ...
    private static func apply(_ event: TaskEvent, to state: inout TaskState, gated: Bool) {
        ...
        if applyVerificationLifecycle(event, to: &state) { return }
        ...
        case .claimed, .progress, .evidence, .verified, .rejected,
             ...
             .verificationStarted, .verificationProcessAttached, .verificationAbandoned:
    ...
    private static func applyVerificationLifecycle(_ event: TaskEvent, to state: inout TaskState) -> Bool {
        switch event.type {
        case .verificationStarted:
            state.pendingVerification = event.verificationLease
            state.verificationProcess = nil
            state.lastCheck = nil
        case .verificationProcessAttached:
            state.verificationProcess = event.verificationProcess
        case .verificationAbandoned:
    ...
    private static func applyOwnerReport(_ event: TaskEvent, to state: inout TaskState) -> Bool {
        ...
        case .claimed:
            ...
            state.missionID = event.missionID
            state.authorityGrantID = event.authorityGrantID
