...
struct PlanMCPAuthority: Codable, Equatable, Sendable {
    enum Operation: String, Codable, Sendable, CaseIterable {
        case read, claim, event, verdict
        // Project-level intake rights are never implied by a task's event grant.
        case bootstrap, research
    }
    ...
    func refusal(
        ...
        if [.claim, .event, .verdict].contains(operation), requestedTaskID != taskID {
