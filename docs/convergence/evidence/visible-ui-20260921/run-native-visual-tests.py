...
label = 'visual-tests-' + datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
def snapshot():
...
scenes = {
 'empty-en': ('ProjectAssistantViewTests/testEmptyStateMatchesSelectedLanguage', 'Assistant empty state', 'en', 'US', 'light'),
 'empty-fr': ('ProjectAssistantViewTests/testEmptyStateMatchesSelectedLanguage', 'Assistant empty state', 'fr', 'FR', 'dark'),
 'assistant-en': ('ProjectAssistantViewTests/testStopDuringStreamKeepsNextTurnIndependent', 'Stopped response and completed next turn', 'en', 'US', 'light'),
 'assistant-fr': ('ProjectAssistantViewTests/testStopDuringStreamKeepsNextTurnIndependent', 'Stopped response and completed next turn', 'fr', 'FR', 'dark'),
 'unknown-fr': ('PlanIntegrationViewTests/testUnknownIsVisibleAndCannotBeRetriedInFrench', 'unknown-fr', 'fr', 'FR', 'dark'),
 'diff-en': ('PlanIntegrationViewTests/testDiffFailureRetryAndEmptyStateInEnglish', 'diff-failed-en', 'en', 'US', 'light'),
}
config = scenes[sys.argv[1]]
...
for item in data['TestConfigurations']:
 for target in item['TestTargets']:
  target.setdefault('EnvironmentVariables', {}).update({'THROTTLE_REVIEW_SCENE':config[1], 'THROTTLE_REVIEW_APPEARANCE':config[4]})
...
command = ['/Applications/Xcode.app/Contents/Developer/usr/bin/xcodebuild', 'test-without-building',
 ...
 '-testLanguage', config[2], '-testRegion', config[3], '-only-testing:ThrottleTests/'+config[0]]
