...
# An unknown feed never authorizes a release build. Compare the candidate with
# every published CFBundleVersion, not only the first item or an exact collision.
PLANNED_BUILD=$(grep -m1 'CURRENT_PROJECT_VERSION:' project.yml \
    | sed -E 's/^[[:space:]]*CURRENT_PROJECT_VERSION:[[:space:]]*"?([^"[:space:]]+)"?[[:space:]]*$/\1/')
PLANNED_VERSION=$(grep -m1 'MARKETING_VERSION:' project.yml | sed 's/.*"\(.*\)".*/\1/')
python3 "$PROJECT_DIR/scripts/verify-release-build.py" --candidate "$PLANNED_BUILD"
